package com.agnieszkarut.foundation.ui.baseview;
    ...
    protected void createLayout() {
        ...
        save.addClickListener(e -> {
            try {
                ...
                clearForm();
                refreshGrid();
                Notification.show(i18n(BaseView.class, "writeSuccess"), DURATION_OK, Notification.Position.MIDDLE);
                UI.getCurrent().navigate(getClass());
            } catch (ObjectOptimisticLockingFailureException exception) {
                Notification.show(i18n(BaseView.class, "simultaneousWrite"), DURATION_ERR, Notification.Position.MIDDLE)
                        .addThemeVariants(NotificationVariant.LUMO_ERROR);
            } catch (ValidationException validationException) {
                Notification.show(i18n(BaseView.class, "validationError"), DURATION_ERR, Notification.Position.MIDDLE);
            } catch (Exception ex) {
                if (ex.getCause() != null && ex.getCause().getCause() instanceof ConstraintViolationException) {
                    ...
                    Notification.show(messages, DURATION_ERR, Notification.Position.MIDDLE);
                } else {
                    Notification.show(i18n(BaseView.class, "writeError"), DURATION_ERR, Notification.Position.MIDDLE)
                            .addThemeVariants(NotificationVariant.LUMO_ERROR);
                }
            }
        });
        ...
        remove.addClickListener(e -> {
            try {
                if (entity == null) {
                    Notification.show(i18n(BaseView.class, "notSelected"), DURATION_OK, Notification.Position.MIDDLE);
                    return;
                }
                service.delete(entity.getId());
                clearForm();
                refreshGrid();
                Notification.show(i18n(BaseView.class, "removeSuccess"), DURATION_OK, Notification.Position.MIDDLE);
                UI.getCurrent().navigate(getClass());
            } catch (ObjectOptimisticLockingFailureException exception) {
                Notification.show(i18n(BaseView.class, "simultaneousWrite"), DURATION_ERR, Notification.Position.MIDDLE)
                        .addThemeVariants(NotificationVariant.LUMO_ERROR);
